...
export function inicializarCatalogo(alAgregarAlCarrito) {
    ...
    // 3. Función interna para renderizar las tarjetas basado en un array recibido [cite: 47]
    const renderizarTarjetas = (listaCafes) => {
        // Limpiamos el contenedor por si había elementos previos o loaders [cite: 43]
        ...
        // Iteramos sobre cada café utilizando el método de array map() de ES6+ [cite: 34]
        listaCafes.map(cafe => {
            // Usamos desestructuración de objetos para extraer las propiedades de forma limpia [cite: 31]
            ...
            // Insertamos la estructura HTML usando Template Literals (comillas invertidas) [cite: 30]
            tarjeta.innerHTML = `
                <div>
                    <h3>${nombre}</h3>
            ...
            // Asignamos el evento al botón de agregar usando addEventListener [cite: 39]
    ...
    const filtrarProductos = () => {
        // Convertimos el texto ingresado a minúsculas para una búsqueda insensible a mayúsculas [cite: 70]
        const textoBusqueda = buscadorInput.value.toLowerCase().trim();
        const origenSeleccionado = filtroOrigen.value;

        // Filtramos usando el método filter() de ES6+ [cite: 34]
        const cafesFiltrados = CAFES_CATALOGO.filter(cafe => {
            // Condición de coincidencia por nombre [cite: 70]
            const coincideNombre = cafe.nombre.toLowerCase().includes(textoBusqueda);
            // Condición de coincidencia por origen del selector [cite: 69]
    ...
    // 5. Escuchamos los eventos de tipeo en el buscador y de cambio en el selector de origen [cite: 39]
